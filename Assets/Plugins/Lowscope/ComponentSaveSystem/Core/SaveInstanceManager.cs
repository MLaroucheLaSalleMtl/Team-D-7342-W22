...
namespace Lowscope.Saving.Core
{
    ...
    public class SaveInstanceManager : MonoBehaviour
    {
        ...
        public SavedInstance SpawnObject(InstanceSource source, string filePath, string customSourcePath = "", string saveIdentification = "", string userTag = "", SpawnSource spawnSource = SpawnSource.FromUserCode)
        {
        ...
        public void OnLoad(SaveGame saveGame)
        {
            ...
            if (saveData != null && saveData.infoCollection != null)
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    var obj = SpawnObject(source, filePath, sourceId, id, savedSpawnInfo.userTag, spawnSource: SpawnSource.FromSave);
