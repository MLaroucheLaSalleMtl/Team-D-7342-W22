...
namespace Lowscope.Saving
{
    ...
    public class SaveMaster : MonoBehaviour
    {
        ...
        // Used to track duplicate scenes & prevent GC from getting the name.
        private static Dictionary<string, int> loadedSceneNames = new Dictionary<string, int>();
        internal static Dictionary<int, SceneInformation> loadedScenes = new Dictionary<int, SceneInformation>();
        private static HashSet<int> multiLoadedScenes = new HashSet<int>();
        ...
        private static HashSet<int> initiallyLoadedScenes = new HashSet<int>();
        ...
        private static void CreateInstance()
        {
            ...
            multiLoadedScenes.Clear();
            loadedScenes.Clear();
            saveables.Clear();
            initiallyLoadedScenes.Clear();
            activeSaveGame = null;
        ...
        internal static string GetSceneName(int sceneInstanceId)
        {
            ...
            if (loadedScenes.TryGetValue(sceneInstanceId, out sceneInfo))
            {
                return sceneInfo.name;
            }
            else
            {
                Debug.LogError("Scene name not found? Have you renamed a scene with saveables in it?");
                return "";
            }
        }
        ...
        */
        ...
        {
            int sceneHash = scene.GetHashCode();
            SceneInformation sceneInfo;

            if (initiallyLoadedScenes.Contains(sceneHash))
            {
                initiallyLoadedScenes.Remove(sceneHash);
            }

            if (!loadedScenes.TryGetValue(sceneHash, out sceneInfo))
            {
                Debug.LogError("No information found about unloaded scene?");
                return;
            }

            if (!sceneInfo.isLoadedMultipleTimes)
            {
                if (loadedSceneNames.ContainsValue(sceneHash))
                {
                    loadedSceneNames.Remove(sceneInfo.name);
                }
            }
            else
            {
                multiLoadedScenes.Remove(sceneHash);
            }

            loadedScenes.Remove(sceneHash);

            if (activeSaveGame == null)
                return;

            SaveInstanceManager instanceManager;
            if (saveInstanceManagers.TryGetValue(sceneHash, out instanceManager))
            {
                if (activeSaveGame != null)
                {
                    instanceManager.OnSave(activeSaveGame);
                }
                saveInstanceManagers.Remove(sceneHash);
            }
        }
        ...
        private static void OnSceneLoaded(Scene scene, LoadSceneMode arg1)
        {
            ...
            sceneInfo.hash = sceneHash;
            sceneInfo.name = sceneName;

            // Store a refeference to a non-duplicate scene
            if (!loadedSceneNames.ContainsKey(sceneName))
            {
                loadedSceneNames.Add(sceneName, sceneHash);
            }
            else
            {
                sceneInfo.isLoadedMultipleTimes = true;
                multiLoadedScenes.Add(sceneHash);
            }

            loadedScenes.Add(sceneHash, sceneInfo);

            if (activeSaveGame == null)
                return;

            // Dont create save instance manager if there are no saved instances in the scene.
            if (string.IsNullOrEmpty(activeSaveGame.Get(string.Format("SaveMaster-{0}-IM", sceneName))))
            {
                return;
            }
        ...
        public static SaveInstanceManager SpawnInstanceManager(Scene scene, string id = "")
        {
            ...
            if (!loadedScenes.TryGetValue(sceneHash, out sceneInfo))
            {
                // Lazy load scene info if it hasn't been loaded yet.
                OnSceneLoaded(scene, LoadSceneMode.Single);
                initiallyLoadedScenes.Add(sceneHash);
                loadedScenes.TryGetValue(sceneHash, out sceneInfo);
            }

            // In case a custom scene ID has been specified, we remove the scene from the multiloaded hashset
            // This is used to allow spawning saved prefabs to the specified scene.
            if (!string.IsNullOrEmpty(id) && multiLoadedScenes.Contains(sceneHash))
            {
                multiLoadedScenes.Remove(sceneHash);
            }

            // Already exists (Might have been created during the lazy load part above), return the current one
            if (saveInstanceManagers.ContainsKey(sceneHash))
            {
                SaveInstanceManager fetchInstanceManager;
                var getSceneManager = saveInstanceManagers.TryGetValue(sceneHash, out fetchInstanceManager);
                return fetchInstanceManager;
            }
            ...
            string sceneID = string.IsNullOrEmpty(id) ? sceneInfo.name : string.Format("{0}-{1}", sceneInfo.name, id);

            saveInstanceManagers.Add(sceneHash, instanceManager);

            var saveID = string.Format("{0}-{1}-IM", "SaveMaster", sceneID);

            instanceManager.SaveID = saveID;
            instanceManager.SceneID = sceneID;
            instanceManager.Scene = scene;
        ...
        {
            return Saveable.IsComponentLoaded(saveable);
        }
        ...
        public static void SetSlotWithoutSave(int slot)
        {
            ...
            SaveMaster.OnLoadingFromDiskDone(slot);
        ...
        public static void WipeSceneData(string name, bool clearSceneSaveables = true)
        {
            ...
            {
                List<Saveable> wipedSaveables = new List<Saveable>();
                ...
                int wipedSaveablesCount = wipedSaveables.Count;
                for (int i = 0; i < wipedSaveablesCount; i++)
                {
        ...
        public static void RemoveListener(Saveable saveable)
        {
            ...
            {
                if (isQuittingGame && !SaveSettings.Get().autoSaveOnExit)
                    return;
        ...
        {
            OnSyncLoadBegin.Invoke(activeSlot);
            ...
            OnSyncLoadDone.Invoke(activeSlot);
        }
        ...
        public static GameObject SpawnSavedPrefab(InstanceSource source, string filePath, string customSource = "", 
            Scene scene = default(Scene), bool clearWhenDisabled = false, string userTag = "")
        {
            ...
            if (multiLoadedScenes.Contains(scene.GetHashCode()))
            {
            ...
            SavedInstance obj = saveIM.SpawnObject(source, filePath, customSourcePath: customSource, userTag : userTag, spawnSource: SaveInstanceManager.SpawnSource.FromUserCode);
        ...
        public static string GetActiveSaveFilePath()
        {
            if (activeSaveGame == null || activeSlot == -1)
            {
                Debug.Log("SaveMaster: failed to obtain path to save game, invalid slot or savegame loaded.");
                return "";
            }
        ...
        {
            get { return instance.onSyncLoadBegin; }
            set { instance.onSyncLoadBegin = value; }
        }

        public static Action<int> OnSyncLoadDone
        {
            get { return instance.onSyncLoadDone; }
            set { instance.onSyncLoadDone = value; }
        }

        public static Action<int> OnLoadingFromDiskBegin
        {
            get { return instance.onLoadingFromDiskBegin; }
            set { instance.onLoadingFromDiskBegin = value; }
        }

        public static Action<int> OnLoadingFromDiskDone
        {
            get { return instance.onLoadingFromDiskDone; }
            set { instance.onLoadingFromDiskDone = value; }
        }
        ...
        {
            get { return instance.onDeletedAllSaves; }
            set { instance.onDeletedAllSaves = value; }
        }
        ...
        private Action<int> onSyncLoadBegin = delegate { };
        private Action<int> onSyncLoadDone = delegate { };

        private Action<int> onSyncSaveBegin = delegate { };
        private Action<int> onSyncSaveDone = delegate { };

        private Action<int> onLoadingFromDiskBegin = delegate { };
        private Action<int> onLoadingFromDiskDone = delegate { };
        ...
        private Action onDeletedAllSaves = delegate { };
